...
export type ExpenseAutoSource = "commission_release" | "monthly_incentive_release"
...
export type Expense = {
  ...
  // Set only on expenses auto-generated by a commission release -- the order ids whose released
  // commission this expense covers. Null on every manually-entered expense. Amount/category/date
  // on these are locked server-side; only Release/Undo Release on the Incentives page changes them.
  commissionOrderIds: string[] | null
  // Set only on expenses auto-generated by a monthly incentive release -- the release row this
  // expense's amount came from. Null on every other expense. Locked the same way as
  // commissionOrderIds; only Release/Undo Release on the Sales-target Bonus tab changes them.
  monthlyIncentiveReleaseId: string | null
  // Derived by the server from the two fields above. Drives the "Auto" badge / locked-field
  // treatment in expense-table.tsx and expense-form-dialog.tsx.
